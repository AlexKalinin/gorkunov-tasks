...
# Table pending_posts
pending_posts_ids = []
$logger.info "#{migration_id}: fill table 'pending_posts' with #{N_POSTS * N_USERS} rows..."
time_before_generation = Time.now
...
for i in 0..(N_POSTS - 1) do
  ...
  for j in 0..(N_USERS - 1) do
    ...
    user_id = users_ids[j]
    query = 'INSERT INTO viewed_posts(post_id, user_id)VALUES ($1, $2);'
    db.exec_params(query, [post_id, user_id])

    pg_res_id = db.exec_query('SELECT last_value FROM pending_posts_id_seq;')
    viewed_posts_ids << pg_res_id.first['last_value'].to_i
  end
end
$logger.info "#{migration_id}: done at #{Time.now - time_before_generation} seconds. " +
                 "Generated #{viewed_posts_ids.count} rows".colorize(:light_cyan)
